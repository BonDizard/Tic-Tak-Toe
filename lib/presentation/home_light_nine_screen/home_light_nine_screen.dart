import '../home_light_nine_screen/widgets/homelightnine_item_widget.dart';import 'controller/home_light_nine_controller.dart';import 'models/homelightnine_item_model.dart';import 'package:bharath_s_application5/core/app_export.dart';import 'package:bharath_s_application5/presentation/home_light_eight_page/home_light_eight_page.dart';import 'package:bharath_s_application5/widgets/custom_bottom_bar.dart';import 'package:bharath_s_application5/widgets/custom_search_view.dart';import 'package:flutter/material.dart';class HomeLightNineScreen extends GetWidget<HomeLightNineController> {const HomeLightNineScreen({Key? key}) : super(key: key);

@override Widget build(BuildContext context) { mediaQueryData = MediaQuery.of(context); return SafeArea(child: Scaffold(resizeToAvoidBottomInset: false, body: Container(width: double.maxFinite, padding: EdgeInsets.symmetric(horizontal: 22.h, vertical: 21.v), child: Column(children: [Align(alignment: Alignment.centerLeft, child: Padding(padding: EdgeInsets.only(left: 34.h), child: Text("lbl_friends".tr, style: CustomTextStyles.headlineSmallMedium))), SizedBox(height: 35.v), CustomSearchView(controller: controller.searchController, hintText: "lbl_player_name".tr), SizedBox(height: 11.v), _buildHomeLightNine()])), bottomNavigationBar: Padding(padding: EdgeInsets.symmetric(horizontal: 24.h), child: _buildBottomBar()))); } 
/// Section Widget
Widget _buildHomeLightNine() { return Expanded(child: Padding(padding: EdgeInsets.only(right: 4.h), child: Obx(() => ListView.separated(physics: BouncingScrollPhysics(), shrinkWrap: true, separatorBuilder: (context, index) {return SizedBox(height: 12.v);}, itemCount: controller.homeLightNineModelObj.value.homelightnineItemList.value.length, itemBuilder: (context, index) {HomelightnineItemModel model = controller.homeLightNineModelObj.value.homelightnineItemList.value[index]; return HomelightnineItemWidget(model, onTapTxtParvez: () {onTapTxtParvez();});})))); } 
/// Section Widget
